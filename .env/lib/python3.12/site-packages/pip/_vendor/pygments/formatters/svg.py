...
class SvgFormatter(Formatter):
    ...
    `fontfamily`
        ...
        attribute, default to ``"monospace"``.

    `fontsize`
        The value to give the wrapping ``<g>`` element's ``font-size``
        attribute, default to ``"14px"``.
    ...
    `xoffset`
        Starting offset in X direction, default to ``0``.

    `yoffset`
        Starting offset in Y direction, default to the font size if it is given
        in pixels, or ``20`` else.  (This is necessary since text coordinates
        refer to the text baseline, not the top edge.)

    `ystep`
        Offset to add to the Y coordinate for each subsequent line.  This should
        roughly be the text size plus 5.  It default to that value if the text
